...
response = requests.post(url, headers=headers, data={**payload, **data}, files=files)
...
Start generating metadata for your images in multiple languages using the `/describe` endpoint and enhance your application's capabilities with the ForVoyez API!
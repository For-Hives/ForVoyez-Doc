...
export const metadata = {
    ...
    description: 'Explore the complete reference of API endpoints, parameters, responses, error codes, and error messages for the ForVoyez API.',
}
...
- `No file uploaded`: The `image` parameter is missing in the request.
- `Invalid image file`: The uploaded file is not a valid image format.
- `Invalid schema`: The provided schema is invalid or malformed.

- `401 Unauthorized`: Authentication failed.
- `Unauthorized, missing Authorization header`: The `Authorization` header is missing in the request.
- `Unauthorized, invalid token`: The provided API key or token is invalid.
- `Unauthorized, no credit left`: The user has insufficient credits to make the request.
